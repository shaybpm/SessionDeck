...
| 12 | **A session that closes** disappears from the normal view and stays available in the card's expanded view (▼) with a resume option. Retention: the last ~20 closed sessions per workspace (`ClosedSessionRetention`). **Changed in this fork (v0.9.101):** ▼ expires after `ExpandedCardTtl` (5 min) and the card collapses itself — being expanded is invisible once the header scrolls away, so a mis-click next to ▶ and ⋯ used to leave closed sessions on a card for the rest of the run, which reads as a deck that failed to retire them. |
